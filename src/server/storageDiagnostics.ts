...
export function logStorageDiagnostics(result: StorageDiagnosticsResult): void {
  ...
  if (looksInsideRepo && result.megaHints.found && result.megaHints.candidates.length > 0) {
    console.warn(
      '[storage] WARNING: Storage path appears to be inside the repo. For MEGA sync, set NEARBYTES_STORAGE_DIR to your MEGA folder (e.g. $HOME/MEGA/NearbytesStorage/NearbytesStorage or a path under: ' +
