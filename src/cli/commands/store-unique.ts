...
export async function handleStoreUnique(options: StoreUniqueOptions): Promise<void> {
  try {
    ...
    if (result.wasDeduplicated) {
      console.log(green('  (Data was deduplicated - reused existing encrypted block)'));
    }
  } catch (error) {
...
export function registerStoreUniqueCommand(program: Command): void {
  program
    ...
    .description('Store data in a channel with deduplication (reuses existing encrypted blocks)')
